...
// import router from '@/router'
const isKeepAlive = data => {
  get(data, 'meta.cache', false)
}
export default {
  namespaced: true,
  state: {
  ...
  actions: {
    ...
    openedLoad ({ state, commit, dispatch }) {
      return new Promise(async resolve => {
        // store 赋值
        const value = await dispatch('db/get', {
          dbName: 'sys',
          path: 'page.opened',
    ...
    open ({ state, commit, dispatch }, { name, params, query, fullPath }) {
      return new Promise(async resolve => {
        ...
        const pageOpendParams = opened.find((page, index) => {
          const same = page.name === name
          pageOpendIndex = same ? index : pageOpendIndex
          return same
        })
        // 判断是否根据参数打开标签
        let pageOpenTarget = name.indexOf('rule-page') ? pageOpendParams : pageOpend
        if (pageOpenTarget) {
          // 页面以前打开过
        ...
        commit('currentSet', { fullPath, params, query })
        // end
        resolve()
      })
    },
  ...
  mutations: {
     ...
     */
    currentSet (state, { fullPath, params, query }) {
